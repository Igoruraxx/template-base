                        ...
                        <div key={s.id} className="flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-muted-foreground flex items-center gap-1">
                              {/* Lookup manual do personal já que o join foi removido por segurança */}
                              via {s.profiles?.full_name?.split(' ')[0] || trainers.find(t => t.user_id === s.trainer_id)?.full_name?.split(' ')[0] || 'Personal'} • {formatDistanceToNow(new Date(s.created_at), { addSuffix: true, locale: ptBR })}
                            </p>
                          </div>
                        </div>
                      ))}
